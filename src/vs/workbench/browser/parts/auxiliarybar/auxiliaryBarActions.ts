	...
	constructor() {
		super({
			...
			menu: [
				...
				{
					id: MenuId.MenubarAppeachanceMenu,
...
class FocuspeachAIExtensionAction extends Action2 {
	static readonly ID = 'workbench.action.focuspeachAIExtension';
	static readonly LABEL = localize2(
		"focuspeachAIExtension",
		"Focus into peachAI Extension",
	);

	constructor() {
		super({
			id: FocuspeachAIExtensionAction.ID,
			title: FocuspeachAIExtensionAction.LABEL,
			category: Categories.View,
			f1: true,
			// keybinding: do not add keybinding CTRL/CMD L here, it comes from peachai extension
		});
	}

	override async run(accessor: ServicesAccessor): Promise<void> {
		// focus peachai extension
		const commandService = accessor.get(ICommandService);
		commandService.executeCommand('peachai.focusContinueInput');
	}
}

registerAction2(FocuspeachAIExtensionAction);
...
// Following is a only peachAI related action, need to refactor these type of actions to separate file
import { IOpenerService } from 'vs/platform/opener/common/opener';
import { URI } from 'vs/base/common/uri';
import { IProductService } from 'vs/platform/product/common/productService';

class OpenpeachAIDocsAction extends Action2 {
	static readonly ID = 'workbench.action.openpeachAIDocs';
	static readonly LABEL = localize2(
		"openpeachAIDocs",
		"Open peachAI Documentation",
	);

	constructor() {
		super({
			id: OpenpeachAIDocsAction.ID,
			title: OpenpeachAIDocsAction.LABEL,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (!productService.peachAILinks?.docs) {
			return;
		}
		await openerService.open(URI.parse(productService.peachAILinks?.docs));
	}
}

registerAction2(OpenpeachAIDocsAction);
